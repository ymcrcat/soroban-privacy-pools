...
use crate::{
    config::TREE_DEPTH,
    crypto::{conversions::*, coin::generate_commitment},
    types::{CoinData, StateFile, AssociationSetFile, SnarkInput},
    ...
    pub fn withdraw_coin(
        ...
        for (index, commitment_str) in state_file.commitments.iter().enumerate() {
            let commitment_fr = decimal_string_to_bls_scalar(env, commitment_str)
                .map_err(|e| CoinUtilsError::InvalidDecimal(format!("Invalid commitment at index {}: {}", index, e)))?;
        ...
        let commitment_index = commitment_index.ok_or_else(|| CoinUtilsError::CommitmentNotFound)?;
        ...
        let siblings: Vec<BlsScalar> = siblings_scalars.iter()
            .map(|s| s.clone())
            .collect();
    ...
    fn handle_association_set(
        ...
        for (index, label_str) in association_set.labels.iter().enumerate() {
            let label_fr = decimal_string_to_bls_scalar(env, label_str)
                .map_err(|e| CoinUtilsError::InvalidDecimal(format!("Invalid association label at index {}: {}", index, e)))?;
        ...
        let association_siblings: Vec<BlsScalar> = association_siblings_scalars.iter()
            .map(|s| s.clone())
            .collect();
