...
pub struct Poseidon255 {
    round_constants: [BlsScalar; 128], // 64 rounds × 2 width for t=2
    mds_matrix: [[BlsScalar; 2]; 2],  // 2x2 matrix for t=2
}
...
impl Poseidon255 {
    ...
    pub fn hash(&self, input: &BlsScalar) -> BlsScalar {
        ...
        // Apply all 64 rounds with correct full/partial structure
        for round in 0..64 {
            // Add round constant
    ...
    /// Hash two inputs by concatenating them into a single input
    pub fn hash_two(&self, input1: &BlsScalar, input2: &BlsScalar) -> BlsScalar {
        // For compatibility with existing code, we'll hash the first input
        // and then use the second as a "salt" by adding it to the result
        let hash1 = self.hash(input1);
        hash1 + input2
    }
