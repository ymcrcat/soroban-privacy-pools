...
// Template for hashing two field elements (simplified)
template Keccak256FieldHash2() {
...
// Template for hashing three field elements (simplified)
template Keccak256FieldHash3() {
    signal input in[3]; // Three field elements
    signal output out;
    
    // Convert field elements to bits (simplified)
    component toBits1 = Num2Bits(254);
    component toBits2 = Num2Bits(254);
    component toBits3 = Num2Bits(254);
    ...
    // Combine bits
    component keccak = Keccak256(762); // 254 * 3
    
    for (var i = 0; i < 254; i++) {
        keccak.in[i] <== toBits1.out[i];
        keccak.in[i + 254] <== toBits2.out[i];
        keccak.in[i + 508] <== toBits3.out[i];
    }
